...
public class Metrics {
    ...
    public Metrics(JavaPlugin plugin, int serviceId) {
        ...
        // Verificăm dacă config-ul este setat, dacă nu, adăugăm valorile implicite
    ...
    private int getPlayerAmount() {
        try {
            ...
            if (onlinePlayersMethod.getReturnType().equals(Collection.class)) {
                return ((Collection<?>) onlinePlayersMethod.invoke(Bukkit.getServer())).size();
            } else {
                return ((Player[]) onlinePlayersMethod.invoke(Bukkit.getServer())).length;
            }
        } catch (Exception e) {
            return Bukkit.getOnlinePlayers().size();  // Compatibilitate fallback pentru versiuni moderne.
        }
    }
